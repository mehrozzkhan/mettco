...
- [x] **Lighthouse mobile** (report: `lighthouse-mobile.report.html`). Localhost production build: **Accessibility 100, Best Practices 100, SEO 100, CLS 0, TBT ≤200ms**. Performance measured 87–89 locally; local runs are depressed by the on-machine image optimizer and CPU contention — re-run against the production URL after deploy and record the number here: `npx lighthouse https://mettco.com.pk --form-factor=mobile --screenEmulation.mobile`.
